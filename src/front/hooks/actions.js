 ...
 * @returns {Promise<any>} Los datos JSON de la respuesta.
 */
const fetchWithAuth = async (url, method = "GET", body = null, token = null) => {
    const options = {
        method: method,
        headers: {
...
export const signup = async (dispatch, body) => {
    try {
        // CORREGIDO: La ruta del backend es /register, no /signup
...
export const fetchMorePosts = async (dispatch, page, perPage = 6) => {
    try {
        const data = await fetchWithAuth(`${BASE_URL}/api/posts?page=${page}&per_page=${perPage}`);
...
export const fetchAllFavorites = async (dispatch, token) => {
    try {
        ...
        const data = await fetchWithAuth(`${BASE_URL}/api/favorites`, "GET", null, token);
...
export const fetchUserPosts = async (dispatch, token, userId) => {
    try {
        const data = await fetchWithAuth(`${BASE_URL}/api/users/${userId}/posts`, "GET", null, token);
...
export const createPost = async (dispatch, token, userId, body) => {
    try {
        const data = await fetchWithAuth(`${BASE_URL}/api/user/post/${userId}`, "POST", body, token);
...
export const deletePost = async (dispatch, token, postId) => {
    try {
        await fetchWithAuth(`${BASE_URL}/api/post/${postId}`, "DELETE", null, token);
...
export const editPost = async (dispatch, token, postId, body) => {
    try {
        const data = await fetchWithAuth(`${BASE_URL}/api/post/${postId}`, "PUT", body, token);
...
export const loadComments = async (dispatch, postId) => {
    try {
        // NOTA: Esta URL asume que el endpoint GET /api/post/:postId/comments existe en el backend.
        const data = await fetchWithAuth(`${BASE_URL}/api/post/${postId}/comments`);
...
export const addComment = async (dispatch, store, commentText, postId) => {
    ...
    try {
        ...
        const data = await fetchWithAuth(`${BASE_URL}/api/post/${postId}/comments`, "POST", body, token);
...
export const toggleCommentLike = async (dispatch, store, commentId) => {
    const { token, user, allComments } = store;
    if (!token || !user) throw new Error("You must be logged in to like a comment.");
    ...
    // REFACTORIZADO: Se usa una sola URL y se cambia el método.
    const method = hasLiked ? "DELETE" : "POST";
    const url = `${BASE_URL}/api/comments/${commentId}/like`; // URL singular '/like'

    try {
        const data = await fetchWithAuth(url, method, null, token);
        if (hasLiked) {
            dispatch({ type: "delete_comment_like", payload: { commentId, userId: user.id } });
        } else {
...
export const deleteComment = async (dispatch, store, commentId) => {
    ...
    try {
        // NOTA: Esta URL asume que el endpoint DELETE /api/comments/:commentId existe en el backend.
        await fetchWithAuth(`${BASE_URL}/api/comments/${commentId}`, "DELETE", null, token);
...
export const toggleFavorite = async (dispatch, store, postId) => {
    ...
    const token = store.token;
    const isFavorited = store.allFavorites.some(fav => fav.post_id === postId && fav.user_id === store.user.id);
    
    let method, url, favoriteId, body;

    if (isFavorited) {
        const favorite = store.allFavorites.find(fav => fav.post_id === postId && fav.user_id === store.user.id);
        if (!favorite) return;
    ...
    try {
        const data = await fetchWithAuth(url, method, body, token);
        if (isFavorited) {
